...
首先登录知乎，将 Headers 中的 Cookies 复制下来，这里可以替换成你自己的 Cookies。

![](/assets/3-2-4.png)

将其设置到 Headers 里面，发送 Request，示例如下：
...
在前面我们提到过 12306 的证书实际上是不被官方认可的，会出现证书验证错误的结果，我们现在访问它都可以看到一个证书问题的页面，如图所示：
...
在前面介绍 Urllib 时我们可以将 Request 表示为一个数据结构，Request 的各个参数都可以通过一个 Request 对象来表示，在Requests 里面同样可以做到，这个数据结构就叫 Prepared Request。
